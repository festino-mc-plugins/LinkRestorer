...
public class CommandParser implements StyleParser
{
	...
	private static void mergeEqualStyles(List<SingleStyleSubstring> substrings)
	{
		for (int i = substrings.size() - 1; i > 0; i--)
		{
			...
			if (substring.endIndex == nextSubstring.beginIndex && substring.style == nextSubstring.style)
			{
				substrings.remove(i);
				substrings.remove(i - 1);
				substrings.add(i - 1, new SingleStyleSubstring(substring.beginIndex, nextSubstring.endIndex, substring.style));
			}
		}
	}
}
